...
构建优化：合并资源，减少请求数量、
压缩（Tree shaking 去除无用的模块）
Terser-webpack-plugin 代替 Uglifyjs
不使用style-loader ==> min-css-extract-plugin

是否把JS合并到一个js文件里？

DNS解析：DNS缓存

复用TCP、升级http到1.1或者2.0，多路复用
...
首次向服务器发送请求，返回资源和缓存标识。
（http1.0 expires 失效时间戳）
(http1.1 cache-control max-age 时间段)
...
if-modified-since/last-modified：资源文件最近一次修改的时间戳

if-none-match/etag：用于表示资源内容是否发生改变 大文件很难计算值

Nginx的负载均衡，导致同一个文件的etag值可能不一样。
